#!/usr/bin/env python3
"""
Plot slim sweep CSVs: rank (x) vs mean throughput kHz (y), one line per CPU.

Throughput = cycle_count / wall_time_s (shown as kHz). Multiple benchmarks → one subplot each
(vertical stack); line hue differs per benchmark, CPUs differ by shade/marker. Optional --benchmark
filters to a single subplot. Optional --cpu plots one CPU only (focus on rank).

Examples:
  python3 plot_slim_sweep_rank_throughput.py --csv slim_sweep_data_memcpy2.csv -o memcpy.pdf
  python3 plot_slim_sweep_rank_throughput.py --csv slim_sweep_data_matmul.csv -o all.pdf
  python3 plot_slim_sweep_rank_throughput.py --csv slim_sweep_data_matmul.csv --benchmark matmul -o matmul.pdf
  python3 plot_slim_sweep_rank_throughput.py --csv slim_sweep_data_matmul.csv --cpu boom21-2large -o one_cpu.pdf
  python3 plot_slim_sweep_rank_throughput.py --csv data.csv --max-rank 10 -o ranks_1_10.pdf
