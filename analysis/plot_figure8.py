...
from analysis import log_parser
import configs
import settings

from analysis import plot_styles
...
def plot_throughput(filename):
    ...
    for design in settings.tested_designs:
        ...
        throughput = None
        try:
            throughput = log_parser.get_throughput_data(baseline_simulator, design, benchmark, 1, settings.iterations)
        except Exception:
            pass
    ...
    for i, sim in enumerate(simulators):
        sim_color = plot_styles.simulator_colors[sim]
        # For each simulator
        ...
        for j, design in enumerate(settings.tested_designs):
            ...
            if baseline_speed[j] is None:
                ...
                throughput = 0
                try:
                    throughput = log_parser.get_throughput_data(sim, design, benchmark, 1, settings.iterations)
                except Exception:
...
if __name__ == '__main__':
    ...
    log_parser.report_rerun_targets()
